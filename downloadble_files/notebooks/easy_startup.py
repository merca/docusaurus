...
def read_df_from_blob_storage(account_name, container_name, file_name, file_type):
    df = spark.read.format(file_type).load(
        "abfss://"
        + container_name
        + "@"
        + account_name
...
df = read_df_from_blob_storage(
    "account_name", "container_name", "file_name", "file_type"
)
...
df.write.saveAsTable(
    name="table_name",
    path="abfss://<container_name>@<account_name>.blob.core.windows.net/folder",
)
